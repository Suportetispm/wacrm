...
function request() {
  return new Request('http://localhost/api/tickets/ticket-1/waiting-customer', { method: 'POST' })
}
...
describe('POST /api/tickets/[id]/waiting-customer', () => {
  ...
  it('calls mark_ticket_waiting_customer with the ticket id, no body needed', async () => {
    mocks.getCurrentAccount.mockResolvedValue(ctx())
    mocks.rpc.mockResolvedValue({
      data: { id: 'ticket-1', status: 'pending' },
      error: null,
    })
    ...
    expect(json.ticket.status).toBe('pending')
    expect(mocks.rpc).toHaveBeenCalledWith('mark_ticket_waiting_customer', { p_ticket_id: 'ticket-1' })
  })

  it("surfaces an invalid-state RPC error (22023, e.g. ticket not 'open' or unassigned) as 400", async () => {
    mocks.getCurrentAccount.mockResolvedValue(ctx())
    mocks.rpc.mockResolvedValue({
      data: null,
      error: { code: '22023', message: 'Ticket must be open (in progress) to wait on the customer' },
    })
