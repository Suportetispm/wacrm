...
// GET one type (any member, RLS-scoped); PATCH edits fields, including
// is_active (archive/reactivate) — never a physical delete, there is
// no DELETE handler on this route at all.
...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await supabase.from('internal_ticket_types').select('*').eq('id', id).maybeSingle()
    if (error) {
      console.error('[internal-tickets/types/[id]] GET failed:', sqlCode(error))
...
export async function PATCH(
  ...
  if (error) {
    ...
    console.error('[internal-tickets/types/[id]] PATCH failed:', code)
