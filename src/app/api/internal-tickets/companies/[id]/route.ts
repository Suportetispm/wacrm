...
// GET one company (any member, RLS-scoped); PATCH edits fields,
// including is_active — never a physical delete.
...
export async function GET(
  ...
  try {
    ...
    const { data, error } = await supabase.from('internal_companies').select('*').eq('id', id).maybeSingle()
    if (error) {
      console.error('[internal-tickets/companies/[id]] GET failed:', sqlCode(error))
...
export async function PATCH(
  ...
  if (error) {
    ...
    console.error('[internal-tickets/companies/[id]] PATCH failed:', code)
