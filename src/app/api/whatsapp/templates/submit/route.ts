...
export async function POST(request: Request) {
  try {
    // Submitting a template calls Meta directly (and burns part of its
    // "100 template creates/hour" quota) before the `message_templates_insert`
    // RLS policy (requires 'admin') ever runs on the row upserted below.
    // Without this upfront check, any authenticated member — agent or
    // viewer — could spend the account's Meta quota with no way to
    // persist the result locally. Same fix as the broadcast route
    // (requireRole), raised to 'admin' to match message_templates_insert.
