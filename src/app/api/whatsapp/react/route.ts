...
export async function POST(request: Request) {
  try {
    // Reacting sends to Meta directly, before the `message_reactions_modify`
    // RLS policy (requires 'agent') ever gets a chance to run on the
    // mirroring write below. Without this upfront check, a 'viewer' could
    // trigger a real reaction on WhatsApp and only fail on the harmless DB
    // write afterwards. Same fix as the broadcast route (requireRole('agent')).
