...
// Maps each peek failure reason to its translation key pair — not
// user-facing text itself, just the lookup table into JoinPage's
// messages namespace.
const FAIL_COPY_KEYS: Record<PeekFail['reason'], { title: string; body: string }> = {
  ...
  server_error: { title: 'failServerErrorTitle', body: 'failServerErrorBody' },
};
...
export default function JoinPage() {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(
            payload.error || t('defaultConflictMessage'),
          );
        } else {
  ...
  // ----- Peek failed -----
  if (!peek.ok) {
    const copyKeys = FAIL_COPY_KEYS[peek.reason];
        ...
        <Card className="w-full max-w-md border-border bg-card">
          ...
          <CardContent className="flex flex-col gap-3">
            ...
            <p className="text-center text-xs text-muted-foreground">
              {t('acceptHintPrefix')}{' '}
              <span className="text-muted-foreground">{peek.account_name}</span>. {t('acceptHintSuffix')}
            </p>
          </CardContent>
        </Card>
